...
async def handle_broadcast_all_message(
    ...
    for user in users:
        try:
            ...
            if not telegram_id:
                logger.warning(
                    f"Пользователь с id {user.id} не имеет telegram_id, пропускаю"
                )
                continue
            elif isinstance(admin_id, int) and telegram_id == admin_id:
                continue
            ...
            if media_group:
                ...
                await message.bot.send_message(chat_id=user.telegram_id, text=text)  # type: ignore
...
async def handle_broadcast_class_message(
    ...
    if not grade:
        logger.error("Не удалось получить класс для рассылки")
        await message.answer(
            "Извините, произошла ошибка. Пожалуйста, попробуйте позже."
        )
        return
