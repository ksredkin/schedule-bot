...
Для управления ботом реализована закрытая админ-панель. Она вызывается командой /admin, которая скрыта из общего меню команд.

**Возможности админки:**
- 📈 Статистика: Общее количество пользователей и распределение по классам.

- 📩 Управление отзывами: Просмотр входящих сообщений, возможность удалять отзывы, помечать их как прочитанные или отправлять ответ пользователю прямо через интерфейс бота.

- 🔐 Безопасность: Доступ защищен кастомным Middleware по Telegram ID.

**Настройка доступа:**
Чтобы панель стала доступна, необходимо указать ваш ID в файле `.env`:

1. Узнайте свой ID через [@userinfobot](https://t.me/userinfobot) или аналоги.
2. Пропишите его в переменную: `ADMIN_ID=ваш_id.`
